...
#include <fstream>
#include <ctime>
//Программа создаёт отпределённое кол-во студентов случайном образом, итог программы находиться в файле Out.bin в папке с проектом
...
int main() {
	...
	cin >> count; cout << endl;
	Table.RandomDataLineCreate(count, "StudentsLinesK_one.bin", "StudentsLinesK_two.bin", "StudentsLinesK_three.bin", "StudentsLinesK_four.bin");
	Table.MaxYearsSearch(count, "StudentsLinesK_one.bin", "StudentsLinesK_two.bin", "StudentsLinesK_three.bin", "StudentsLinesK_four.bin", "Out.bin");
}
